...
export default function IndustryProcess({ data }: Props) {
  const title = data?.title || "Как протича изработката?";
  const items = data?.items || sharedProcess;
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 lg:grid-cols-5 gap-16 lg:gap-10 relative z-10">
            {items.map((step, idx) => (
              <div key={idx} className="flex flex-col items-center text-center group">
                ...
                <h3 className="font-black text-xs uppercase tracking-[0.2em] mb-3 text-foreground/90 leading-tight min-h-[2.5em] flex items-center">
                  {step.title}
                </h3>
